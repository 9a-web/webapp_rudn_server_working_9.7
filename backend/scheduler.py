...
class NotificationScheduler:
    ...
    async def _check_and_notify(
        ...
        try:
            ...
            # 2. Текущее время < времени уведомления + 1.5 минуты (окно для отправки)
            # Это окно предотвращает пропуск уведомлений при небольших задержках
            if -0.5 <= time_diff_minutes < 1.5:
